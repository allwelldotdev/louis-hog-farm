...
It depends what kind of record. A hog's own details — including fixing its recorded parents — can be edited; the system checks that the change still makes sense (a mother must be female, a father male, an animal can't be its own parent, and a parent must have been born before its offspring) before it accepts it. A breeding cycle can be closed out, and an alert can be acknowledged or resolved. But a vaccination record or a death record, once entered, cannot be edited or deleted — that's deliberate, not an oversight, so that history stays trustworthy. Weigh-ins, health checks and feed records currently have no edit option either; if one's wrong, the fix is to log a corrected follow-up entry rather than change the original.
...
Both exist now. The phone app is where day-to-day recording happens — weigh-ins, health checks, feed, vaccinations — plus a quick look at headline numbers and one weight chart per animal; anything deeper points you back to the dashboard, which is where the fuller analysis, charts, and farm settings live, from a computer browser in a dark or light colour scheme. One honest caveat: the phone app has now been used on a real phone by the developer — installed on an actual Android device, signed in, and used to record a real weigh-in that showed up on the dashboard automatically, with no reload needed — but it hasn't yet been used day-to-day on a working farm. And as above, it needs to reach the farm's computer over the farm's own wifi to save anything; there's no working-offline mode.
